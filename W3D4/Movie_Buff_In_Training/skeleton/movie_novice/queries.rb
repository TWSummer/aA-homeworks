...
def below_average_years
  ...
  query = Movie.select("movies.yr, COUNT(*) AS bad_movies")
  query = query.where("score < 5")
  query = query.group("movies.yr")
...
def alphabetized_actors
  ...
  query = Actor.select("actors.id, actors.name")
  query = query.order("actors.name ASC")
...
def pulp_fiction_actors
  ...
  query = Actor.select("actors.id, actors.name")
  query = query.joins(:movies)
  query.where("movies.title = 'Pulp Fiction'")
...
def uma_movies
  ...
  query = Movie.select("movies.id, movies.title, movies.yr")
  query = query.joins(:actors)
  query = query.where("actors.name = 'Uma Thurman'")
  query.order("movies.yr ASC")
end
